/**
 * @fileoverview Error handling utilities for Codex CLI provider
 */

import { APICallError, LoadAPIKeyError } from '@ai-sdk/provider';

/**
 * @typedef {import('./types.js').CodexCliErrorMetadata} CodexCliErrorMetadata
 */

/**
 * Create an API call error with Codex CLI specific metadata
...
export function createTimeoutError({ message, promptExcerpt, timeoutMs }) {
	...
	/** @type {CodexCliErrorMetadata & { timeoutMs: number }} */
	const metadata = {
		code: 'TIMEOUT',
		promptExcerpt,
		timeoutMs
...
export function isAuthenticationError(error) {
	...
	if (
		...
		/** @type {CodexCliErrorMetadata} */ (error.data)?.exitCode === 401
	)
		return true;
	return false;
}
...
export function isTimeoutError(error) {
	if (
		error instanceof APICallError &&
		/** @type {CodexCliErrorMetadata} */ (error.data)?.code === 'TIMEOUT'
	)
		return true;
	return false;
}
 ...
 * @returns {CodexCliErrorMetadata|undefined}
 */
export function getErrorMetadata(error) {
	if (error instanceof APICallError && error.data) {
		return /** @type {CodexCliErrorMetadata} */ (error.data);
	}
	return undefined;
}
