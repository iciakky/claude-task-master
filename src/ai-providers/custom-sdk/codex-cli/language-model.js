/**
 * @fileoverview Codex CLI Language Model implementation
 */

import { NoSuchModelError } from '@ai-sdk/provider';
import { generateId } from '@ai-sdk/provider-utils';
import { convertToCodexCliMessages } from './message-converter.js';
...
async function loadCodexCliModule() {
	if (!query || !AbortError) {
		try {
			const mod = await import('@openai/codex-cli');
			query = mod.query;
 ...
 * @typedef {import('./types.js').CodexCliLanguageModelOptions} CodexCliLanguageModelOptions
 */
...
export class CodexCliLanguageModel {
	specificationVersion = 'v1';
	defaultObjectGenerationMode = 'json';
	supportsImageUrls = false;
	supportsStructuredOutputs = false;

	/** @type {CodexCliModelId} */
	modelId;
	...
	/**
	 * @param {CodexCliLanguageModelOptions} options
	 */
	constructor(options) {
	...
	/**
	 * Get the model name for Codex CLI
	 * @returns {string}
	 */
	getModel() {
		...
		if (unsupportedParams.length > 0) {
			...
			for (const param of unsupportedParams) {
				warnings.push({
					...
					setting: param,
					details: `Codex CLI does not support the ${param} parameter. It will be ignored.`
				});
			}
		}
	...
	async doGenerate(options) {
		await loadCodexCliModule();
		const { messagesPrompt } = convertToCodexCliMessages(
			options.prompt,
			options.mode
		);
		...
		const queryOptions = {
			...
			pathToCodexCliExecutable: this.settings.pathToCodexCliExecutable,
		...
		try {
			...
			if (isJsonTruncation && text && text.length > 0) {
				warnings.push({
					...
					details:
						'Codex CLI SDK emitted a JSON parse error but Task Master recovered buffered text (possible CLI truncation).'
				});
				...
				if (
					...
					throw createAuthenticationError({
						message:
							error.message ||
							'Authentication failed. Please ensure Codex CLI is properly authenticated.'
					});
				}

				// Wrap other errors with API call error
				throw createAPICallError({
					message: error.message || 'Codex CLI error',
					code: error.code,
				...
				try {
					...
					if (
						...
						controller.enqueue({
							...
							finishReason: 'truncated',
							usage,
							providerMetadata: { 'codex-cli': { truncated: true } },
							warnings: [
								{
									type: 'provider-warning',
									details:
										'Codex CLI SDK JSON truncation detected; stream recovered.'
								}
							]
						});
