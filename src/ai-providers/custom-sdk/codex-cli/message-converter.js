/**
 * @fileoverview Converts AI SDK prompt format to Codex CLI message format
 */

/**
 * Convert AI SDK prompt to Codex CLI messages format
 ...
 */
export function convertToCodexCliMessages(prompt, mode) {
	const messages = [];
	let systemPrompt;
		...
		switch (message.role) {
			...
			case 'user':
				if (typeof message.content === 'string') {
					...
					// Note: Image parts are not supported by Codex CLI
					const imageParts = message.content.filter(
						(part) => part.type === 'image'
					);
					if (imageParts.length > 0) {
						console.warn(
							'Codex CLI does not support image inputs. Images will be ignored.'
						);
					}
				}
				break;
